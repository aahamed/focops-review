...
from models import GaussianPolicy, Value, DiscretePolicy
...
class FOCOPS:
    ...
    def __init__(self,
        ...
        self.pi_scheduler = pi_scheduler
        self.vf_scheduler = vf_scheduler
        self.cvf_scheduler = cvf_scheduler
        ...
        if is_discrete_action_space(self.env):
    ...
    def update_params(self, rollout, dtype, device):
        ...
        if not self.discrete_action:
            ...
            old_logprob, old_mean, old_std = to_dytype_device(
                    dtype, device, old_logprob, old_mean, old_std)
            old_logprob, old_mean, old_std = graph_detach(old_logprob, old_mean, old_std)
            # dummy prob
            old_prob = torch.zeros(old_logprob.shape)
        else:
            old_logprob, old_prob = self.policy.logprob(obs, act)
            old_logprob, old_prob = to_dytype_device(dtype, device,
                    old_logprob, old_prob)
            old_logprob, old_prob = graph_detach(old_logprob, old_prob)
            # dummy mean and std
            old_mean = torch.zeros(old_logprob.shape)
            old_std = torch.zeros(old_logprob.shape)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b, cvtarg_b, cadv_b,
                    old_logprob_b, old_mean_b,
                    old_std_b, old_prob_b) in enumerate(loader):
                ...
                if not self.discrete_action:
                    ...
                    logprob, prob = self.policy.logprob(obs_b, act_b)
                    kl_new_old = discrete_kl(prob, old_prob_b)
                ratio = torch.exp(logprob - old_logprob_b)
                self.pi_loss = (kl_new_old - (1 / self.lam) * ratio 
                        * (adv_b - self.nu * cadv_b)) \
            ...
            if not self.discrete_action:
                ...
                logprob, prob = self.policy.logprob(obs, act)
                kl_val = discrete_kl(prob, old_prob).mean().item()
        ...
        num_eps = rollout['num_eps']
        avg_eps_len = rollout['avg_eps_len']
        avg_speed_per_timestep = rollout['avg_speed_per_timestep']
        std_speed_per_timestep = rollout['std_speed_per_timestep']
        # get number of episodes that violated cost constraint
        # by more than 10% in the last 100 eps
        vio_lim = 1.1 * self.cost_lim
        num_vio = ( np.array( self.cscore_queue ) > vio_lim ).sum()
        ...
        self.logger.update('StdR', np.std(self.score_queue))
        ...
        self.logger.update('StdC', np.std(self.cscore_queue))
        self.logger.update('NumE', num_eps)
        self.logger.update('AvgEl', avg_eps_len)
        self.logger.update('NumV', num_vio)
        self.logger.update('nu', self.nu)
        self.logger.update('score_queue', self.score_queue)
        self.logger.update('cscore_queue', self.cscore_queue)
        self.logger.update('avg_speed_per_timestep', avg_speed_per_timestep)
        self.logger.update('std_speed_per_timestep', std_speed_per_timestep)
        ...
        self.logger.save_model('pi_scheduler', self.pi_scheduler.state_dict())
        self.logger.save_model('vf_scheduler', self.vf_scheduler.state_dict())
        self.logger.save_model('cvf_scheduler', self.cvf_scheduler.state_dict())
...
def train(args):
    ...
    # Initialize policy based on action_space
    policy = None
    if not discrete_action:
        policy = GaussianPolicy(obs_dim, act_dim, args.hidden_size, args.activation, args.logstd)
    else:
        policy = DiscretePolicy(obs_dim, act_dim, args.hidden_size, args.activation)
    ...
    # load state if resuming from checkpoint
    current_iter = 0
    env_id = args.env_id
    exp_name = args.exp
    exp_dir = f'focops_results/{env_id}/{exp_name}'
    if args.resume: 
        def get_file( target, exp_dir ):
            files = os.listdir(exp_dir)
            t_file = [ f for f in files if target in f ][0]
            t_file = os.path.join( exp_dir, t_file )
            assert t_file
            return t_file

        # get saved data
        hp = pickle.load( open( get_file( 'hyper', exp_dir ), "rb" ) )
        log_data = pickle.load( open( get_file( 'log_data', exp_dir ), "rb" ) )
        models = torch.load( get_file( 'models', exp_dir ) )

        # update information
        current_iter = models['iter'] + 1
        hyperparams = hp
        logger.hyperparams = hyperparams
        logger.log_data = log_data
        logger.models = models
        logger.iter = current_iter
        policy.load_state_dict( models['policy_params'] )
        value_net.load_state_dict( models['value_params'] )
        cvalue_net.load_state_dict( models['cvalue_params'] )
        pi_optimizer.load_state_dict( models['pi_optimizer'] )
        vf_optimizer.load_state_dict( models['vf_optimizer'] )
        cvf_optimizer.load_state_dict( models['cvf_optimizer'] )
        pi_scheduler.load_state_dict( models['pi_scheduler'] )
        vf_scheduler.load_state_dict( models['vf_scheduler'] )
        cvf_scheduler.load_state_dict( models['cvf_scheduler'] )
        running_stat = log_data['running_stat']
        score_queue = log_data['score_queue']
        cscore_queue = log_data['cscore_queue']
    else:
        os.makedirs( exp_dir, exist_ok=True )
        assert not os.listdir(exp_dir)

    setup_logger( exp_dir )
    log( f'Exp Name: {exp_name} Env: {env_id}' )
    log( f'hyperparams: {hyperparams}\n\n' )
    
    # Get constraint bounds
    cost_lim = get_threshold(envname, hyperparams, constraint=args.constraint)

    # Initialize and train FOCOPS agent
    agent = FOCOPS(env, policy, value_net, cvalue_net,
                   pi_optimizer, vf_optimizer, cvf_optimizer,
                   pi_scheduler, vf_scheduler, cvf_scheduler,
    ...
    for iter in range(current_iter, args.max_iter_num):
        # import pdb; pdb.set_trace()
        sleep(5)
        ...
        data_generator = DataGenerator(obs_dim, act_dim, args.batch_size, args.max_eps_len, cost_lim, discrete_action)
