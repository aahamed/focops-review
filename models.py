...
class DiscretePolicy(nn.Module):
    ...
    def forward(self, obs):
        # import pdb; pdb.set_trace()
        out = self.mlp_net(obs)
        act = self.action_head(out)
        act = torch.softmax(act, dim=-1)
        return act

    def get_act(self, obs, deterministic = False):
        pred_act = self.forward(obs)
        if deterministic:
            return pred_act.argmax(dim=1)
        else:
            return pred_act.multinomial(num_samples=1)

    def logprob(self, obs, act):
        pred_act = self.forward(obs)
        return torch.log(pred_act.gather(1, act.long())), pred_act
